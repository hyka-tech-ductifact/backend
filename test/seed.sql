...
INSERT INTO orders (id, title, status, project_id, created_at, updated_at) VALUES
  ('d0000000-0000-0000-0000-000000000001', 'Steel beams – lot 3',         'pending',   'p0000000-0000-0000-0000-000000000001', NOW() - INTERVAL '5 days', NOW() - INTERVAL '5 days'),
  ('d0000000-0000-0000-0000-000000000002', 'Concrete mix – delivery 1',   'completed', 'p0000000-0000-0000-0000-000000000001', NOW() - INTERVAL '3 days', NOW() - INTERVAL '3 days'),
  ('d0000000-0000-0000-0000-000000000003', 'Electrical wiring – phase A', 'pending',   'p0000000-0000-0000-0000-000000000001', NOW() - INTERVAL '1 day',  NOW() - INTERVAL '1 day');

-- ─── Orders (Alice → Acme Corp → Office Park Norte) ────────
-- 2 orders for the second project — verifies isolation between projects.

INSERT INTO orders (id, title, status, project_id, created_at, updated_at) VALUES
  ('d0000000-0000-0000-0000-000000000004', 'HVAC units – building A',     'pending',   'p0000000-0000-0000-0000-000000000002', NOW() - INTERVAL '3 days', NOW() - INTERVAL '3 days'),
  ('d0000000-0000-0000-0000-000000000005', 'Elevator installation',       'completed', 'p0000000-0000-0000-0000-000000000002', NOW() - INTERVAL '1 day',  NOW() - INTERVAL '1 day');

-- ─── Orders (Alice → Globex → Port Expansion Phase 2) ──────
-- 1 order with minimal data — tests defaults.

INSERT INTO orders (id, title, status, project_id, created_at, updated_at) VALUES
  ('d0000000-0000-0000-0000-000000000006', 'Container crane rental',      'pending',   'p0000000-0000-0000-0000-000000000004', NOW(),                      NOW());

-- ─── Orders (Bob → Oscorp Labs → Genetics Lab Expansion) ───
-- 2 orders for Bob — verifies that Alice cannot see Bob's orders.

INSERT INTO orders (id, title, status, project_id, created_at, updated_at) VALUES
  ('d0000000-0000-0000-0000-000000000007', 'Gene sequencer – model X',    'pending',   'p0000000-0000-0000-0000-000000000007', NOW() - INTERVAL '1 day',  NOW() - INTERVAL '1 day'),
  ('d0000000-0000-0000-0000-000000000008', 'Lab bench furniture',         'completed', 'p0000000-0000-0000-0000-000000000007', NOW(),                      NOW());
