...
// Returns (orderID, defID, token).
func createFullChainForPieces(t *testing.T) (string, string, string) {
	t.Helper()
	_, _, projectID, token := createUserClientAndProject(t, "Juan", "juan@example.com", "Acme Corp", "Tower B")

	// Create order
	orderResp := helpers.AuthPostJSON(t, orderURL(projectID), token, map[string]string{
	...
	return orderID, defID, token
}

// pieceURL builds the full URL for piece endpoints under an order.
func pieceURL(orderID string, extra ...string) string {
	base := "/orders/" + orderID + "/pieces"
	if len(extra) > 0 {
		base += "/" + extra[0]
	}
	return url(base)
...
func TestE2E_CreatePiece_NoToken_Returns401(t *testing.T) {
	clean(t)
	orderID, defID, _ := createFullChainForPieces(t)

	resp := helpers.PostJSON(t, pieceURL(orderID), map[string]any{
		"title":         "Panel",
		"definition_id": defID,
		"dimensions":    map[string]float64{"Length": 100, "Width": 50},
		"quantity":      1,
...
func TestE2E_CreatePiece_WrongUser_Returns404(t *testing.T) {
	...
	resp := helpers.AuthPostJSON(t, pieceURL(orderID), token2, map[string]any{
		"title":         "Panel",
		"definition_id": defID,
		"dimensions":    map[string]float64{"Length": 100, "Width": 50},
		"quantity":      1,
...
func TestE2E_CreatePiece_NonExistingOrder_Returns404(t *testing.T) {
	...
	resp := helpers.AuthPostJSON(t, pieceURL("00000000-0000-0000-0000-000000000000"), token, map[string]any{
		"title":         "Panel",
		"definition_id": defID,
		"dimensions":    map[string]float64{"Length": 100, "Width": 50},
		"quantity":      1,
...
func TestE2E_ListPieces_Success(t *testing.T) {
	...
	r1 := helpers.AuthPostJSON(t, pieceURL(orderID), token, map[string]any{
	...
	r2 := helpers.AuthPostJSON(t, pieceURL(orderID), token, map[string]any{
	...
	resp := helpers.AuthGetJSON(t, pieceURL(orderID), token)
...
func TestE2E_GetPiece_Success(t *testing.T) {
	...
	createResp := helpers.AuthPostJSON(t, pieceURL(orderID), token, map[string]any{
	...
	resp := helpers.AuthGetJSON(t, pieceURL(orderID, pieceID), token)
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	body := helpers.ParseBody(t, resp)
...
func TestE2E_GetPiece_NotFound_Returns404(t *testing.T) {
	...
	resp := helpers.AuthGetJSON(t, pieceURL(orderID, "00000000-0000-0000-0000-000000000000"), token)
	assert.Equal(t, http.StatusNotFound, resp.StatusCode)
}
...
func TestE2E_GetPiece_InvalidID_Returns400(t *testing.T) {
	...
	resp := helpers.AuthGetJSON(t, pieceURL(orderID, "not-a-uuid"), token)
	assert.Equal(t, http.StatusBadRequest, resp.StatusCode)
}
...
func TestE2E_GetPiece_WrongUser_Returns404(t *testing.T) {
	...
	resp := helpers.AuthGetJSON(t, pieceURL(orderID, pieceID), token2)
	assert.Equal(t, http.StatusNotFound, resp.StatusCode)
}
...
func TestE2E_UpdatePiece_Success(t *testing.T) {
	...
	resp := helpers.AuthPutJSON(t, pieceURL(orderID, pieceID), token, map[string]any{
		"title":      "New Title",
...
func TestE2E_UpdatePiece_NotFound_Returns404(t *testing.T) {
	...
	resp := helpers.AuthPutJSON(t, pieceURL(orderID, "00000000-0000-0000-0000-000000000000"), token, map[string]any{
		"title": "Ghost",
	})
...
func TestE2E_UpdatePiece_WrongUser_Returns404(t *testing.T) {
	...
	resp := helpers.AuthPutJSON(t, pieceURL(orderID, pieceID), token2, map[string]any{
		"title": "Stolen",
	})
...
func TestE2E_DeletePiece_Success(t *testing.T) {
	...
	resp := helpers.AuthDeleteJSON(t, pieceURL(orderID, pieceID), token)
	assert.Equal(t, http.StatusNoContent, resp.StatusCode)

	// Verify it's gone
	getResp := helpers.AuthGetJSON(t, pieceURL(orderID, pieceID), token)
...
func TestE2E_DeletePiece_NotFound_Returns404(t *testing.T) {
	...
	resp := helpers.AuthDeleteJSON(t, pieceURL(orderID, "00000000-0000-0000-0000-000000000000"), token)
	assert.Equal(t, http.StatusNotFound, resp.StatusCode)
}
...
func TestE2E_DeletePiece_WrongUser_Returns404(t *testing.T) {
	...
	resp := helpers.AuthDeleteJSON(t, pieceURL(orderID, pieceID), token2)
	assert.Equal(t, http.StatusNotFound, resp.StatusCode)
}
...
func TestE2E_Piece_FullFlow_Create_Get_Update_List_Delete(t *testing.T) {
	...
	// 3. Update
	updateResp := helpers.AuthPutJSON(t, pieceURL(orderID, pieceID), token, map[string]any{
	...
	listResp := helpers.AuthGetJSON(t, pieceURL(orderID), token)
	...
	// 5. Delete
	deleteResp := helpers.AuthDeleteJSON(t, pieceURL(orderID, pieceID), token)
	assert.Equal(t, http.StatusNoContent, deleteResp.StatusCode)

	// 6. List — should be empty
	listResp2 := helpers.AuthGetJSON(t, pieceURL(orderID), token)
